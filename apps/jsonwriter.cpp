...
wxJSONWriter::WriteIntValue(wxOutputStream& os, const wxJSONValue& value) {
    ...
    wxASSERT(data);
        ...
        snprintf(buffer, sizeof buffer, "%" wxLongLongFmtSpec "d",
    ...
    #endif
#else
    snprintf(buffer, sizeof buffer, "%ld", data->m_value.m_valLong);
#endif
    len = strlen(buffer);
    os.Write(buffer, len);
    if (os.GetLastError() != wxSTREAM_NO_ERROR)    {
...
wxJSONWriter::WriteUIntValue(wxOutputStream& os, const wxJSONValue& value) {
    ...
    #if wxCHECK_VERSION(2, 9, 0) || !defined(wxJSON_USE_UNICODE)
        // this is fine for wxW 2.9 and for wxW 2.8 ANSI
        snprintf(buffer, sizeof buffer, "%" wxLongLongFmtSpec "u",
    ...
    #endif
#else
    snprintf(buffer, sizeof buffer, "%lu", data->m_value.m_valULong);
#endif
    len = strlen(buffer);
    os.Write(buffer, len);
    if (os.GetLastError() != wxSTREAM_NO_ERROR)    {
