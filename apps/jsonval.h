...
class WXDLLIMPEXP_JSON wxJSONValue {
 ...
 public:
    // ctors and dtor
    wxJSONValue();
    explicit wxJSONValue(wxJSONType type);
    explicit wxJSONValue(int i);
    explicit wxJSONValue(unsigned int i);
    explicit wxJSONValue(short i);
    explicit wxJSONValue(unsigned short i);
    explicit wxJSONValue(long int i);
    explicit wxJSONValue(unsigned long int i);
#if defined(wxJSON_64BIT_INT)
    explicit wxJSONValue(wxInt64 i);
    explicit wxJSONValue(wxUint64 ui);
#endif
    explicit wxJSONValue(bool b);
    explicit wxJSONValue(double d);
    explicit wxJSONValue(const wxChar* str);     // assume static ASCIIZ strings
    explicit wxJSONValue(const wxString& str);
    explicit wxJSONValue(const wxMemoryBuffer& buff);
