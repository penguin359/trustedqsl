...
wxJSONValue::AsInt() const {
    wxJSONRefData* data = GetRefData();
    wxJSON_ASSERT(data);
    int i = static_cast<int>(data->m_value.VAL_INT);
...
wxJSONValue::AsCString(wxChar* ch) const {
    bool r = IsCString();
    if (r)    {
        ch = const_cast<wxChar*>(AsCString());
    }
    return r;
}
...
wxJSONValue::Size() const {
    ...
    int size = -1;
    if (data->m_type == wxJSONTYPE_ARRAY)  {
        size = static_cast<int>(data->m_valArray.GetCount());
    }
    if (data->m_type == wxJSONTYPE_OBJECT)  {
        size = static_cast<int>(data->m_valMap.size());
    }
    return size;
}
